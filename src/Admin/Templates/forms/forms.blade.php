...
@section('admin-body')
<div id="phoenix-admin" class="forms-container grid-container">
    <form name="admin-form" id="admin-form" action="{{ $router->html('forms') }}" method="POST" enctype="multipart/form-data">
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <th style="min-width: 90px;">
                        {!! $grid->sortTitle('admin.form.field.state', 'form.state') !!}
                    </th>

                    {{-- TITLE --}}
                    <th>
                        {!! $grid->sortTitle('admin.form.field.title', 'form.title') !!}
                    </th>

                    {{-- ORDERING --}}
                    <th width="5%" class="nowrap">
                        {!! $grid->sortTitle('admin.form.field.ordering', 'form.ordering') !!} {!! $grid->saveorderButton() !!}
                    </th>

                    {{-- AUTHOR --}}
                    <th>
                        {!! $grid->sortTitle('admin.form.field.author', 'form.created_by') !!}
                    </th>

                    {{-- CREATED --}}
                    <th>
                        {!! $grid->sortTitle('admin.form.field.created', 'form.created') !!}
                    </th>

                    {{-- LANGUAGE --}}
                    <th>
                        {!! $grid->sortTitle('admin.form.field.language', 'form.language') !!}
                    </th>

                    {{-- ID --}}
                    <th>
                        {!! $grid->sortTitle('admin.form.field.id', 'form.id') !!}
                    </th>
                </tr>
                </thead>
                ...
                @foreach ($items as $i => $item)
                    ...
                    <tr>
                        ...
                        {{-- TITLE --}}
                        <td>
                            <a href="{{ $router->html('form', array('id' => $item->id)) }}">
                                {{ $item->title }}
                            </a>
                        </td>
                ...
                <tr>
                    {{-- PAGINATION --}}
                    <td colspan="25">
                        {!! $pagination->render($package->getName() . ':forms', 'windwalker.pagination.phoenix') !!}
                    </td>
                </tr>
                </tfoot>
            </table>
