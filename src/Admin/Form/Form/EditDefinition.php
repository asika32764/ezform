...
use Admin\Field\Form\FormListField;
use Admin\Field\Form\FormModalField;
use Phoenix;
	...
	public function define(Form $form)
	{
		...
		$form->wrap('basic', null, function(Form $form)
		{
			...
			$form->add('title', new Field\TextField)
				->label(Translator::translate('admin.form.field.title'))
				->setFilter('trim')
				->required(true);

			// Alias
			$form->add('alias', new Field\TextField)
				->label(Translator::translate('admin.form.field.alias'));

			// Images
			$form->add('images', new Field\TextField)
				->label(Translator::translate('admin.form.field.images'));

			// URL
			$form->add('url', new Field\TextField)
				->label(Translator::translate('admin.form.field.url'))
				->setValidator(new Rule\UrlValidator)
				->set('class', 'validate-url');

			// Example: Form List
			$form->add('form_list', new FormListField)
				->label('List Example');

			// Example: Form Modal
			$form->add('form_modal', new FormModalField)
				->label('Modal Example');
		});
		...
		$form->wrap('text', null, function(Form $form)
		{
			...
			$form->add('introtext', new Field\TextareaField)
				->label(Translator::translate('admin.form.field.introtext'))
				->set('rows', 10);

			// Fulltext
			$form->add('fulltext', new Field\TextareaField)
				->label(Translator::translate('admin.form.field.fulltext'))
				->set('rows', 10);
		});
		...
		$form->wrap('created', null, function(Form $form)
		{
			...
			$form->add('state', new Field\RadioField)
				->label(Translator::translate('admin.form.field.state'))
				->set('class', 'btn-group')
				->set('default', 1)
			...
			$form->add('version', new Field\TextField)
				->label(Translator::translate('admin.form.field.version'));

			// Created
			$form->add('created', new Phoenix\Field\CalendarField)
				->label(Translator::translate('admin.form.field.created'));

			// Modified
			$form->add('modified', new Phoenix\Field\CalendarField)
				->label(Translator::translate('admin.form.field.modified'));

			// Author
			$form->add('created_by', new Field\TextField)
				->label(Translator::translate('admin.form.field.author'));

			// Modified User
			$form->add('modified_by', new Field\TextField)
				->label(Translator::translate('admin.form.field.modifiedby'));
		});
	}
}
