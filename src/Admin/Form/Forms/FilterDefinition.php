...
/**
 * The FormsFilterDefinition class.
 *
 * @since  1.0
 */
	...
	public function define(Form $form)
	{
		...
		$form->wrap(null, 'search', function (Form $form)
		{
			...
			$form->add('field', new ListField)
				...
				->addOption(new Option(Translator::translate('admin.form.field.title'), 'form.title'))
				->addOption(new Option(Translator::translate('admin.form.field.alias'), 'form.alias'));
		 ...
		 *
		 * You can override filter actions in FormsModel::configureFilters()
		 */
		$form->wrap(null, 'filter', function(Form $form)
		{
			// State
			$form->add('form.state', new ListField)
				->label('State')
				// Add empty option to support single deselect button
				->addOption(new Option('', ''))
				->addOption(new Option(Translator::translate('admin.form.filter.state.select'), ''))
