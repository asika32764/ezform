...
/**
 * The EntriesFilterDefinition class.
 *
 * @since  1.0
 */
	...
	public function define(Form $form)
	{
		...
		$form->wrap(null, 'search', function (Form $form)
		{
			...
			$form->add('field', new ListField)
				...
				->addOption(new Option(Translator::translate('admin.entry.field.title'), 'entry.title'))
				->addOption(new Option(Translator::translate('admin.entry.field.alias'), 'entry.alias'));
		 ...
		 *
		 * You can override filter actions in EntriesModel::configureFilters()
		 */
		$form->wrap(null, 'filter', function(Form $form)
		{
			// State
			$form->add('entry.state', new ListField)
				->label('State')
				// Add empty option to support single deselect button
				->addOption(new Option('', ''))
				->addOption(new Option(Translator::translate('admin.entry.filter.state.select'), ''))
